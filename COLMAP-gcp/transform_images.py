...
def trans_images_txt(images_path, new_images_path, R_o2n, t_o2n):
    ...
    for i, param in enumerate(images_param_and_ori):
        ...
        # 2、生成新的旋转四元数和平移矩阵
        # 不妨认为R_o是描述相机坐标系相对于世界坐标系，也就是Pc = R * Pw + t, 其实就是相机坐标系还是c1，但是有两个世界坐标系，w1和w2，然后根据公式推就可以了。
...
parser.add_argument("-o", "--ori_images_path", default = "/home/zt/Project/COLMAP_GroundControlPoints-main/output/txt_outs/images.txt", help = "经过GCP特征点提取调整过之后的images文件位置")
...
# images_path = os.path.join(args.project_folder,"sparse/model/0/images.txt")
images_path = args.ori_images_path
if not os.path.exists(os.path.join(args.project_folder, "sparse/model/manual")):
    os.mkdir(os.path.join(args.project_folder, "sparse/model/manual"))
new_images_path = os.path.join(args.project_folder,"sparse/model/manual/images.txt")

if __name__ == "__main__":
    R_o2n, t_o2n = read_R_t(args.align_output_path)
